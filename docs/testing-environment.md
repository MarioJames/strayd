...
专项测试可通过前述 `scripts/test-env.ts` 命令在具备对应系统能力的机器上运行。Linux 托管 runner 上，`scripts/ci-native.ts` 将夹具测试放入独立 systemd scope，保持原用户身份，避免夹具继承 runner 的服务归属；scope 运行结束后自动回收。

报告由 always artifact 步骤上传，清理失败使运行失败。GitHub runner 的实际 OS 镜像版本仍随每次报告记录；六架构构建成功不能证明最低 macOS/Windows/glibc 版本兼容。
