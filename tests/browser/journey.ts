...
for (const language of ['en', 'zh-cn']) {
  ...
  const ruleType = language === 'en' ? 'PROTECT: keep visible' : '保护：保持可见';
  await until(`window.straydTest.screen().includes(${JSON.stringify(ruleType)})`, 'protection rule in settings');
  await browser('screenshot', join(evidence, `${language}-protection-settings.png`));
  await browser('press', 'u');
  await browser('press', 'Escape');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(protectedLabel)})`, 'protection removed');
  await browser('press', 's');
  ...
  results.push({ language, status: 'pass', sizes: [[132, 40], [100, 18], [80, 24]], copied_full_command: true, cancelled_stop: true, protection_saved_blocked_and_removed: true, exited: 0 });
}
