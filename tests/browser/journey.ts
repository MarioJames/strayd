...
for (const language of ['en', 'zh-cn']) {
  ...
  const protect = language === 'en' ? 'CREATE PROTECTION RULE' : '创建保护规则';
  const protectedLabel = language === 'en' ? 'PROTECTED' : '受保护';
  await until(`window.straydTest.screen().includes(${JSON.stringify(protect)})`, 'protection editor');
  await browser('press', 'Enter');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(protect)}) && window.straydTest.screen().includes(${JSON.stringify(protectedLabel)})`, 'saved protection keeps process visible');
  await browser('press', 'r');
  await until(`window.straydTest.screen().includes('PID ${fixture[0].pid}') && window.straydTest.screen().includes(${JSON.stringify(protectedLabel)})`, 'protection survives refresh');
  await browser('press', 's');
  const blocked = language === 'en' ? 'cannot be stopped as a group' : '拒绝整组关闭';
  await until(`window.straydTest.screen().includes(${JSON.stringify(blocked)})`, 'protected stop rejected');
  assert.equal(await evaluate(`window.straydTest.screen().includes(${JSON.stringify(confirmation)})`), false);
  await browser('screenshot', join(evidence, `${language}-protected.png`));
  await browser('press', 'h');
  await until(`window.straydTest.screen().includes(${JSON.stringify(hide)})`, 'hide protected resource');
  await browser('press', 'Enter');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(hide)}) && !window.straydTest.screen().includes('PID ${fixture[0].pid}') && window.straydTest.screen().includes('PID ${fixture[1].pid}')`, 'protected resource hidden');
  await browser('press', ',');
  await browser('press', 'ArrowDown');
  const hideType = language === 'en' ? '▌ #2 HIDE' : '▌ #2 隐藏';
  await until(`window.straydTest.screen().includes(${JSON.stringify(hideType)})`, 'hide rule selected');
  await browser('press', 'u');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(hideType)})`, 'hide rule removed');
  await browser('press', 'Escape');
  const settings = language === 'en' ? 'SETTINGS / RULES' : '设置 / 规则';
  await until(`!window.straydTest.screen().includes(${JSON.stringify(settings)})`, 'settings closed after unhiding');
  await browser('press', 'Home');
  const unprotect = language === 'en' ? 'UNPROTECT' : '解除保护';
  await until(`window.straydTest.screen().includes('PID ${fixture[0].pid}') && window.straydTest.screen().includes(${JSON.stringify(protectedLabel)}) && window.straydTest.screen().includes(${JSON.stringify(unprotect)})`, 'unhiding retains protection and offers removal');
  const unprotectPosition = await evaluate(`(()=>{const t=window.straydTest.term;const r=document.querySelector('.xterm-screen').getBoundingClientRect();return {x:r.x+r.width*27/t.cols,y:r.y+(t.rows-1.5)*r.height/t.rows}})()`);
  await browser('mouse', 'move', String(Math.round(unprotectPosition.x)), String(Math.round(unprotectPosition.y)));
  await browser('mouse', 'down'); await browser('mouse', 'up');
  const unprotectConfirmation = language === 'en' ? 'CONFIRM UNPROTECT' : '确认解除保护';
  await until(`window.straydTest.screen().includes(${JSON.stringify(unprotectConfirmation)})`, 'unprotect confirmation');
  assert.equal(await evaluate(`window.straydTest.screen().includes(${JSON.stringify(settings)})`), false);
  await browser('click', '[data-cols="80"]');
  await until(String.raw`window.straydTest.screen().split('\n').some(line => line.includes(${JSON.stringify(unprotectConfirmation)}) && line.indexOf(${JSON.stringify(unprotectConfirmation)}) < 20)`, 'unprotect dialog reflows to 80 columns');
  await browser('screenshot', join(evidence, `${language}-80x24-unprotect.png`));
  await browser('press', 'Escape');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(unprotectConfirmation)})`, 'cancel unprotect');
  await browser('click', '[data-cols="132"]');
  await browser('press', 's');
  await until(`window.straydTest.screen().includes(${JSON.stringify(blocked)})`, 'cancel retains protection');
  await browser('press', 'p');
  await until(`window.straydTest.screen().includes(${JSON.stringify(unprotectConfirmation)})`, 'P opens unprotect confirmation');
  await browser('press', 'Enter');
  await until(`!window.straydTest.screen().includes(${JSON.stringify(unprotectConfirmation)}) && !window.straydTest.screen().includes(${JSON.stringify(protectedLabel)})`, 'protection removed directly');
  await browser('click', '[data-cols="132"]');
  await browser('press', 's');
  await until(`window.straydTest.screen().includes(${JSON.stringify(confirmation)})`, 'stop restored after removing protection');
  await browser('press', 'Escape');
  ...
  results.push({ language, status: 'pass', sizes: [[132, 40], [100, 18], [80, 24]], copied_full_command: true, cancelled_stop: true, protection_saved_blocked_and_removed: true, protected_resource_hidden_and_restored: true, unprotect_confirmed_and_cancelled: true, exited: 0 });
}
